...
def campus_gst_questions(course_code: str, username: Optional[str] = Query(None), db=Depends(get_db)):
    # Free tier: 1 mock test per day. GST premium (plan containing "gst") is unlimited.
    ...
    if username:
        ...
        if row:
            ...
            if not gst_unlocked:
                today = today_str()
                attempts_today = db.query(GSTResult).filter(
                    GSTResult.username == username,
                    GSTResult.taken_at.like(f"{today}%"),
                ).count()
                if attempts_today >= 1:
                    raise HTTPException(
                        status_code=402,
                        detail="Daily free mock test used. Upgrade to GST Mock Premium for unlimited tests.",
                    )
...
def payment_initialize(payload: PaymentInitSchema):
    ...
    amount_kobo = payload.amount * 100  # convert Naira -> kobo for Paystack
...
def ai_check_limit(username: str, app_type: str = Query(...), db=Depends(get_db)):
    ...
    # a single unified premium tier) unlocks unlimited AI Assistant use.
    ai_unlocked = is_premium and (app_type != "campus" or (premium_plan and "ai" in premium_plan))

    limit = 999 if ai_unlocked else 2
    can_send = True if ai_unlocked else requests_today < limit
