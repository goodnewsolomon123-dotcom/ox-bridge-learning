...
def is_math_heavy(prompt: str) -> bool:
    """
    Detects whether a prompt is likely to involve math, chemistry, or physics
    notation - used to prioritize the providers that are most reliable at
    following the $ notation formatting instruction (Gemini, Groq, DeepSeek),
    rather than risking a weaker provider answering first and skipping the
    formatting entirely.
    """
    triggers = [
        "solve", "calculate", "equation", "formula", "balance the", "derivative",
        "integral", "algebra", "geometry", "trigonometry", "physics", "chemistry",
        "mathematics", "math ", "theorem", "prove that", "simplify", "factorize",
        "quadratic", "logarithm", "matrix", "vector", "velocity", "acceleration",
        "molarity", "mole", "reaction", "compound", "  ^", "sqrt", "percentage of"
    ]
...
def get_ai_response(prompt: str) -> str:
    ...
    # For math/chemistry/physics-heavy prompts, try the providers most
    # reliable at following the $ notation instruction first (Gemini, Groq,
    # DeepSeek all handle formatting instructions well) - rather than the
    # default speed-first order, which could let a weaker provider answer
    # first and skip proper math formatting entirely.
    if is_math_heavy(prompt):
        provider_order = [_try_gemini, _try_groq, _try_deepseek, _try_openrouter, _try_mistral, _try_cohere, _try_huggingface]
    else:
