...
def get_ai_response(prompt: str) -> str:
    ...
    if OR_KEY and not result:
        ...
        free_models = ["openrouter/free", "meta-llama/llama-4-maverick:free", "meta-llama/llama-3.3-70b-instruct:free", "qwen/qwen3-coder:free", "google/gemma-3-12b-it:free"]
    ...
    if HF_KEY and not result:
        models = ["meta-llama/Llama-3.1-8B-Instruct:novita"]
        for model in models:
            try:
                res = requests.post(
...
def ai_chat(data: AIChatMessage, db=Depends(get_db)):
    ...
    Whenever you write any mathematical expression, equation, or formula - even a
    simple one like x^2 - wrap it in single dollar signs like this: $x^2 + y^2 = z^2$
    (or double dollar signs like $$...$$ for a standalone equation on its own line).
    Never write math in plain text or with ASCII symbols like ^ or sqrt() instead.
    ...
    as if that's simply how you naturally write math.
    {datetime_block}
    {search_block}
...
def ai_chat_vision(data: AIVisionChatSchema, db=Depends(get_db)):
    ...
    Whenever you write any mathematical expression, equation, or formula, always
    wrap it in $...$ for inline math or $$...$$ for a standalone equation - do this
    silently, without ever mentioning the word "LaTeX" or explaining the notation.
