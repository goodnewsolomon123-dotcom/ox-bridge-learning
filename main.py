...
def admin_add_course_question(data: CampusCourseQuestionCreate, db=Depends(get_db)):
...
def admin_add_course_questions_bulk(data: BulkCampusCourseQuestionsCreate, db=Depends(get_db)):
    added = []
    for qd in data.questions:
        q = CampusCourseQuestion(
...
def admin_add_gst_question(data: GSTQuestionCreate, db=Depends(get_db)):
    q = GSTQuestion(
        course_code=data.course_code.strip().upper(), course_title=data.course_title.strip(),
        question_text=data.question_text.strip(),
        option_a=data.option_a.strip(), option_b=data.option_b.strip(),
...
def admin_add_gst_questions_bulk(data: BulkGSTQuestionsCreate, db=Depends(get_db)):
    added = []
    for qd in data.questions:
        q = GSTQuestion(
            course_code=qd.course_code.strip().upper(), course_title=qd.course_title.strip(),
