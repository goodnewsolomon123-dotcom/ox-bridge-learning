...
def campus_gst_questions(course_code: str, db=Depends(get_db)):
...
def ai_check_limit(username: str, app_type: str = Query(...), db=Depends(get_db)):
    ...
    row = db.execute(
        text(f"SELECT is_premium, ai_requests_today, ai_requests_date FROM {table} WHERE username = :username"),
        {"username": username},
    ).fetchone()
    ...
    is_premium, requests_today, requests_date = row
    today = today_str()

    if requests_date != today:
        requests_today = 0

    limit = 999 if is_premium else 2
    can_send = True if is_premium else requests_today < limit
